...
import { fetchAssignedGrievances } from "../store/actions/assignedGrievances-actions";
...
const AssignedGrievancesPage = () => {
  ...
  let tableContent = <p>No Assigned Grievances</p>;
          ...
          {tableState.filteredTableContent
            ...
            .map((grievance, index) => (
              <tr key={index}>
                ...
                <td>{new Date(grievance.deadline).toLocaleString()}</td>
                <td>
  ...
  return (
    <Fragment>
      <div>
        <h1 style={{ marginBottom: "25px" }}>Assigned Grievances</h1>
        <div className="'row">
